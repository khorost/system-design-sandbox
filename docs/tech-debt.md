...
## TD-013: Пользовательские компоненты со скриптовой логикой

**Приоритет:** Medium
**Компоненты:** apps/web, packages/component-library, apps/server
**Зависимости:** TD-004 (каталог схем — для публикации шаблонов)

### Описание

Пользователи создают собственные типы компонентов с настраиваемыми параметрами, визуальным стилем и скриптовой логикой поведения при симуляции. Готовые шаблоны можно сохранять локально и публиковать в общий каталог.

### Шаг 1. Шаблон компонента (Custom Component Template)

Пользователь задаёт через UI-форму:
- Название, иконка (эмодзи или SVG), категория
- Цвет рамки/фона
- Набор конфиг-полей (имя, тип, значение по умолчанию)
- Порты ввода/вывода (количество, подписи)
- Является ли контейнером (может содержать другие узлы)
...
interface CustomComponentTemplate {
  ...
  category: 'custom';
  isContainer: boolean;
  params: CustomParam[];               // пользовательские конфиг-поля
  ports: { inputs: number; outputs: number };
  script?: string;                     // мини-DSL скрипт (шаг 2)
  version: number;
  author?: string;
  description?: string;
  tags?: string[];
...
- [ ] Тип `CustomComponentTemplate` и `CustomParam`
- [ ] UI: модалка «Create Component» — форма с полями name, icon, color, params
- [ ] Предпросмотр узла в реальном времени при редактировании шаблона
- [ ] Хранение шаблонов в localStorage (`sds-custom-components`)
- [ ] Секция «Custom» в палитре компонентов с пользовательскими шаблонами
- [ ] Drag-and-drop пользовательских компонентов на канвас
- [ ] Редактирование и удаление существующих шаблонов
- [ ] Экспорт/импорт шаблонов в JSON (обмен между пользователями)

### Шаг 2. Скриптовая логика (мини-DSL)

Декларативный мини-язык для описания поведения узла при симуляции. Безопасный — без доступа к DOM, сети, файлам. Исполняется в simulation-engine.

#### Синтаксис

```
on request:
  if input.rps > config.max_rps:
    respond error_rate: 0.3
    forward to: "fallback"
  else if headers.region == "eu":
    forward to: "eu-backend"
  else:
    forward to: "us-backend"

on tick:
  repeat config.retry_count:
    if downstream.healthy:
      break
    wait 100ms
```

#### Контекст исполнения (доступные переменные)

| Переменная | Тип | Описание |
|-----------|-----|----------|
| `input.rps` | number | Текущий входящий RPS |
| `input.latency` | number | Входящая задержка (ms) |
| `config.*` | any | Значения конфиг-полей шаблона |
| `downstream.healthy` | boolean | Все ли downstream-узлы живы |
| `downstream.count` | number | Количество downstream-узлов |
| `upstream.rps` | number | Суммарный входящий RPS от upstream |
| `self.utilization` | number | Текущая утилизация (0..1) |
| `self.replicas` | number | Количество активных реплик |

#### Доступные действия

| Действие | Описание |
|---------|----------|
| `forward to: "label"` | Направить запрос на downstream-узел по имени |
| `respond error_rate: N` | Установить долю ошибок (0..1) |
| `respond latency: N` | Добавить задержку (ms) |
| `drop` | Отбросить запрос |
| `wait Nms` | Пауза в цикле (только внутри `repeat`) |
| `log "message"` | Запись в лог симуляции |

#### Конструкции

- `if / else if / else` — ветвление по условию
- `repeat N:` — цикл с ограничением итераций
...
- Максимум итераций на цикл: 100
- Таймаут исполнения: 5ms на тик
- Максимальная вложенность `if`: 5 уровней
- Нет доступа к глобальным объектам (window, document, fetch)
- Нет определения функций и переменных — только чтение контекста и действия

#### Задачи

- [ ] Грамматика мини-DSL (PEG или recursive descent парсер)
- [ ] Парсер `parseScript(source: string) → AST | ParseError`
- [ ] Интерпретатор `executeScript(ast, context) → Action[]`
- [ ] Валидация: проверка типов, существования downstream-имён
- [ ] Интеграция с simulation-engine: вызов скрипта на каждом тике/запросе
- [ ] UI: редактор скрипта в модалке шаблона (textarea с подсветкой синтаксиса)
- [ ] Подсветка ошибок парсинга в реальном времени
- [ ] Кнопка «Test Script» — сухой прогон с моковым контекстом

### Шаг 3. Каталог шаблонов (Community)

Публикация пользовательских компонентов в общий каталог (расширение TD-004).
...
| Метод | Путь | Описание |
|-------|------|----------|
| `POST` | `/api/templates` | Публикация шаблона |
| `GET` | `/api/templates` | Каталог с поиском и фильтрацией |
| `GET` | `/api/templates/:slug` | Получение шаблона |
| `PUT` | `/api/templates/:slug` | Обновление (по edit_token) |
| `DELETE` | `/api/templates/:slug` | Удаление (по edit_token) |

#### Задачи

- [ ] Backend: миграция `component_templates`, CRUD API
- [ ] UI: секция «Community» в палитре компонентов
- [ ] Поиск и фильтрация по тегам, имени, популярности
- [ ] Кнопка «Publish» в редакторе шаблона
- [ ] «Install» — добавление чужого шаблона в локальную коллекцию
- [ ] Версионирование: при обновлении шаблона схемы сохраняют ссылку на конкретную версию
- [ ] Обратная совместимость: если шаблон удалён из каталога, узлы на канвасе работают по последней закешированной версии

### Порядок реализации

```
Шаг 1 (шаблоны)  →  Шаг 2 (DSL-скрипты)  →  Шаг 3 (каталог)
      ↓                    ↓
 Локальное              Интеграция с
 использование          simulation-engine
```
